...
<div class="col-md-10">
	...
	<a href="addResult.jsp" class="btn btn-info btn-sm">Add Assignment
		Result</a> <a href="viewStudentResult.jsp" class="btn btn-info btn-sm">Back
		to Assignment</a>
	...
	<table class="table">
		...
		<tbody>
			<%
				...
				List<ExamResult> ers = erDao.getAllExamResults(courseCode, (onPage - 1) * pageSize, pageSize);
				if (instID != null && instID != "") {
					for (ExamResult er : ers) {
			%>
			<tr>
	...
	<a href="viewExamResult.jsp?page=<%=1%>" class="btn btn-info btn-sm">First</a>
	<%
		for (int i = 1; i <= noOfPages; i++) {
	%>
	<a href="viewExamResult.jsp?page=<%=i%>" class="btn btn-info btn-sm"><%=i%></a>
	<%
		}
	%>
	<a href="viewExamResult.jsp?page=<%=noOfPages%>"
